...
BANNED_USERS_PATH = os.path.join(HELPER_FILES_DIR_PATH, "banned_users.txt")
...
def get_banned_users(banned_users_path) -> list|None:
    if not os.path.exists(banned_users_path):
        return None
...
def create_df(data: list[dict], unique_users_per_skill: dict, skill: str, use_each_n=None, bars_visible=10) -> pd.DataFrame:
    ...
    banned_users = get_banned_users(BANNED_USERS_PATH)
    ...
    def is_valid_frame(frame_num, num_frames):
        ...
        return (use_each_n is None) or (frame_num % use_each_n == 0) or (frame_num == num_frames-1)
...
def create_bar_race(df, bars_visible):
    ...
    video_file_path = os.path.join(BAR_RACE_VIDEOS_DIR, f"bar_race_{time_now}.mp4")
    bcr.bar_chart_race(
        df,
        ...
        n_bars=bars_visible,
        dpi=120,
        interpolate_period=True, # only use this if ALL the data points are equally-spaced!
        period_length=1200,
        steps_per_period=72,
        ...
        bar_label_size=18,
        tick_label_size=22,
        period_label={'x': .99, 'y': .17, 'ha': 'right', 'va': 'center', 'size': '50', 'color': 'indianred'},
        period_fmt='%b %-d, %Y\n%H:%M',
...
def main():
    ...
    print(f"Bar race video saved to: {bar_race_video}")
    print(time.time() - t1)
